...
router.post("/login", async (req, res) => {
  let { email, password } = req.body; // Extract the email and password from the request body

  try {
      // Find the admin by email from the database
      ...
      if (valid && admin) {
          ...
          let token = jwt.sign({ email: admin.email, admin: true }, jwtKey); // Sign the JWT token
          ...
          return res.redirect("/admin/login"); // Stay on the login page
      }
  } catch (err) {
